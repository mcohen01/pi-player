...
//////////////// Editable options ////////////////
	...
	$tutorialTitle = 'Effective Characteristics of Instructional Programs';
...
<center></center></>
<p>
	These 10 tutorials teach research-supported effective characteristics and features of educational software, particularly those that relate to computer-based tutorial instruction.
  ...
  session_start();
    ...
    function readtutorialLine(&$frames, $line, &$frame) {
        ...
        if (strpos(trim($line), '@answer') === 0) {
            $thisAnswer = str_replace("'", "'", trim(substr(trim($line), 7)));
            if (! $frame['answer']) {
                $frame['answer'] = array();
            }
        ...
        if ($isFrame === 1) {
            if (strlen(trim($line)) && trim($line) != '@begin') {
                $frame['frame'] = $frame['frame'].str_replace("'", "'", trim($line)).'<br>';
            } else {
